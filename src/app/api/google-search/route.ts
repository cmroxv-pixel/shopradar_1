...
function getMarketplaceName(url: string): string {
  try {
    ...
    const map: Record<string, string> = {
      ...
      'cashconverters.com.au': 'Cash Converters',
...
function scoreResult(url: string, title: string, snippet: string, price: number): number {
  ...
  // Strong product page signals — high score
  if (/\/itm\/\d+/.test(fullUrl)) score += 100;        // eBay item
  if (/\/dp\/[A-Z0-9]{10}/.test(fullUrl)) score += 100; // Amazon product
  if (/\/ip\//.test(fullUrl)) score += 80;              // Walmart item
  if (/\/product\//.test(fullUrl)) score += 70;
  if (/\/p\/[a-z0-9-]+$/.test(fullUrl)) score += 70;
  if (/\/shop\/.+\/.+\/.+/.test(fullUrl)) score += 70; // Cash Converters deep path
  if (/\/listing\/\d+/.test(fullUrl)) score += 70;
  if (/\d{8,}/.test(path)) score += 60;                // Long numeric ID in URL
  if (/\/buy\//.test(fullUrl)) score += 60;
  if (/\/item\//.test(fullUrl)) score += 60;

  // Price found — big bonus
  if (price > 0) score += 80;
  ...
  if (text.includes('condition:')) score += 20;        // eBay condition field

  // Bad signals — penalise
  ...
  if (/\/shop\/?$/.test(fullUrl)) score -= 100;        // bare /shop page
...
function isValidProductPage(url: string, score: number): boolean {
  // Must have a positive score to be considered a real product page
...
export async function GET(req: NextRequest) {
  ...
  try {
    // Run 5 searches in parallel with different keywords to maximise results
    const searchQueries = [
      `"${query}" buy`,
      `"${query}" price`,
      `"${query}" for sale`,
    ...
    // Score, filter and build listings
    const listings = uniqueResults
      .map((item: any, idx: number) => {
        ...
        let score = scoreResult(url, title, snippet, price);
      ...
      // Filter 1: must be a valid product page with positive score
      .filter((l) => isValidProductPage(l.listingUrl, l.score))
      // Filter 2: must have a price above 0
      .filter((l) => l.price > 0)
      // Sort by score descending — best product pages first
