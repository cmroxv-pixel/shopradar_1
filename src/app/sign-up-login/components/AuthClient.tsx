...
export default function AuthClient() {
  ...
  return (
    ...
    >
      ...
      {/* Base dim grid */}
      <div className="absolute inset-0 z-0" style={{ opacity: 0.06, color: 'white' }}>
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} />
      </div>

      {/* Mouse-reveal bright grid */}
      <motion.div className="absolute inset-0 z-0" style={{ maskImage, WebkitMaskImage: maskImage, opacity: 0.45, color: 'white' }}>
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} />
      </motion.div>
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 400, padding: '0 20px' }}>
        ...
        {/* Blur layer — smears grid behind card */}
        <div style={{
          position: 'absolute', inset: 0, borderRadius: 32, zIndex: 0,
          backdropFilter: 'blur(40px) brightness(1.5) saturate(1.4)',
        ...
        {/* Content */}
        <div style={{
          position: 'relative', zIndex: 2, borderRadius: 32, padding: '36px 28px 32px', overflow: 'hidden',
          boxShadow: [
          ...
          {/* Top gloss */}
          <div aria-hidden style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 70, background: 'radial-gradient(ellipse 80% 90% at 50% -5%, rgba(255,255,255,0.24) 0%, transparent 70%)', pointerEvents: 'none' }} />
          ...
          <AnimatePresence mode="wait">
            <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18 }}>
              ...
              <form onSubmit={tab === 'login' ? handleLogin : handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {tab === 'signup' && <input className="auth-inp" type="text" placeholder="Full name" value={name} onChange={e => setName(e.target.value)} required style={inp} />}
                ...
                {tab === 'signup' && <input className="auth-inp" type="password" placeholder="Confirm password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required style={inp} />}
