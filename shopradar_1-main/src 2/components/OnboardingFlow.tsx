...
import { X, Search, Bookmark, Bell, ChevronRight, ChevronLeft, Check } from 'lucide-react';
      ...
      <div className="relative z-10 w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 px-5 pt-4">
          {STEPS.map((_, i) => (
            <div
              key={`step-dot-${i}`}
              className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'bg-primary w-6' : i < step ? 'bg-primary/40 w-3' : 'bg-muted w-3'}`}
            />
          ))}
        </div>
        ...
        <div className="flex items-center justify-between px-5 pb-5 gap-3">
          <button
            ...
            disabled={step === 0}
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted border border-border transition-all duration-150 disabled:opacity-30 disabled:cursor-not-allowed"
          >
          ...
          <button
            onClick={handleNext}
            className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-semibold bg-primary text-white hover:opacity-90 active:scale-95 transition-all duration-150 shadow-sm shadow-primary/20"
          >
            {isLast ? (
