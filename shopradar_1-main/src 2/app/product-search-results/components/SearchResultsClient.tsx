...
export default function SearchResultsClient() {
  ...
  const saveRecentSearch = useCallback((term: string) => {
    try {
      ...
      const updated = [term, ...saved.filter((s) => s.toLowerCase() !== term.toLowerCase())].slice(
        0,
        6
      );
  ...
  const fetchResults = useCallback(async (name: string, country: string): Promise<Listing[]> => {
    try {
      ...
      const res = await fetch(`/api/serpapi/search?${params}`, { headers });
      if (!res.ok) return [];
  ...
  const handleSearch = useCallback(
    async (overrideQuery?: string) => {
      ...
      posthog.capture('product_searched', { query: name, country: address.country });
      try {
  ...
  const toggleCompare = useCallback((listing: Listing) => {
    setCompareItems((prev) => {
      const alreadyIn = !!prev.find((l) => l.id === listing.id);
      if (alreadyIn) {
        posthog.capture('product_compare_toggled', {
          action: 'removed',
  ...
  const sorted = [...listings]
    .sort((a, b) => {
      ...
      if (sortBy === 'proximity')
        return (a.distanceKm ?? a.deliveryDays * 500) - (b.distanceKm ?? b.deliveryDays * 500);
      return b.sellerRating - a.sellerRating;
    })
    .filter((l) => {
      if (l.stockStatus === 'Out of Stock' || (l.stockStatus as string) === 'Unavailable')
        return false;
    ...
    <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '48px 24px 120px' }}>
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: resultsOpen ? '340px 1fr' : '1fr',
            gap: 32,
            alignItems: 'start',
          }}
        >
          ...
          <div
            style={{
              position: resultsOpen ? 'sticky' : 'relative',
              top: resultsOpen ? 80 : 'auto',
            }}
          >
            {/* Location */}
            ...
            >
              ...
              <input
                ...
                onFocus={() => {
                  if (!searchText && recentSearches.length > 0) setShowRecent(true);
                }}
            ...
            {showRecent && recentSearches.length > 0 && (
              ...
              >
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      localStorage.removeItem('shopradar_recent');
                      setRecentSearches([]);
                      setShowRecent(false);
                    }}
                    style={{
            ...
            >
              Scanning Amazon · eBay · JB Hi-Fi · Kogan
              <br />
              Cash Converters · CeX · Harvey Norman &amp; more
            </p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {SORT_OPTIONS.map((s) => (
                      <button
                        ...
                        onClick={() => {
                          setSortBy(s.id);
                          posthog.capture('sort_option_changed', { sort_by: s.id, query });
                        }}
                        style={{
                          textAlign: 'left',
                          padding: '7px 10px',
                          ...
                          cursor: 'pointer',
                          background:
                            sortBy === s.id ? 'hsl(var(--primary) / 0.08)' : 'transparent',
                          color:
                            sortBy === s.id
                              ? 'hsl(var(--primary))'
          ...
          {resultsOpen && hasSearched && (
            <div className="animate-fade-up">
              <ResultsGrid
                ...
                onAddToWatchlist={(l) => {
                  posthog.capture('product_added_to_watchlist', {
