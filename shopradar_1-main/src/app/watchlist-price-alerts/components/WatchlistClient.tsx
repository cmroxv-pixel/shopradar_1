...
export default function WatchlistClient() {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setWatchlist((prev) =>
              prev.map((item) =>
                item.id === payload.new.id ? dbToWatchlistItem(payload.new) : item
              )
            );
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setAlerts((prev) =>
              prev.map((alert) =>
                alert.id === payload.new.id ? dbToPriceAlert(payload.new) : alert
              )
            );
          } else if (payload.eventType === 'DELETE') {
  ...
  const handleRemoveWatchlist = async (id: string) => {
    ...
    posthog.capture('watchlist_item_removed', { product: item?.productName });
  ...
  const handleDeleteAlert = async (id: string) => {
    ...
    posthog.capture('price_alert_deleted', {
  ...
  const handleToggleAlertStatus = async (id: string) => {
    ...
    posthog.capture('price_alert_toggled', {
      product: alert.productName,
      status: next,
      target_price: alert.targetPrice,
    });
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground max-w-xs">
          Create an account or sign in to save products and set price alerts that persist across
          sessions.
        </p>
        <Link
          href="/sign-up-login"
      ...
      {selectedWl.length > 0 && activeTab === 'watchlist' && (
        <div className="flex items-center justify-between bg-primary/10 border border-primary/30 rounded-xl px-4 py-3">
          <span className="text-sm font-medium text-primary">
            {selectedWl.length} item{selectedWl.length > 1 ? 's' : ''} selected
          </span>
          <div className="flex items-center gap-2">
            <button
