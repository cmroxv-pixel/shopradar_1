...
import { sanitiseString, sanitiseUrl, badRequest, LIMITS } from '@/lib/validate';
...
export async function GET(req: NextRequest) {
  ...
  if (!apiKey) return NextResponse.json({ error: 'Missing Perplexity API key' }, { status: 500 });
...
- Return ONLY the raw JSON array — no explanation, no markdown, no code fences`;
  ...
  try {
    ...
    if (!Array.isArray(raw)) return NextResponse.json({ listings: [] });

    const listings = raw
      .slice(0, 100) // cap total listings
      .filter((l: any) => {
      ...
      .map((l: any) => ({
        ...
        productName: String(l.title || '').slice(0, 300),
        model: '',
        color: '',
        price: parseFloat(String(l.price)),
        originalPrice: parseFloat(String(l.price)),
        currency: sanitiseString(l.currency, 3)?.toUpperCase() ?? 'USD',
        marketplace: String(l.marketplace || 'Unknown').slice(0, LIMITS.MARKETPLACE_MAX),
        marketplaceLogo: '🛒',
        listingUrl: sanitiseUrl(l.url) ?? '',
        condition: ['New', 'Used', 'Refurbished', 'Vintage'].includes(l.condition) ? l.condition : 'New',
        location: String(l.location || '').slice(0, 100),
      ...
      .filter(l => l.listingUrl !== ''); // drop any that failed URL sanitisation
